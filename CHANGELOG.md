...
### Fixed
- **Search now finds terms containing `_` or `%` on SQLite, and works on Postgres.** `DefaultMessageSearch::applyToQuery()` backslash-escaped the wildcards but never declared an `ESCAPE` character, and SQLite has no default, so a search for `john_doe` matched the backslash literally and returned nothing. The needle is now escaped with `!` and every driver gets an explicit `ESCAPE '!'` clause, which is portable across SQLite, MySQL, Postgres and SQL Server. On Postgres the columns are cast to text and matched with `ILIKE`, since `LIKE` is case-sensitive there, so `matches()` on the file driver and `applyToQuery()` on the database driver now agree. Fixes #93.
- **`message_id` is now recorded on real sends, so Message-ID dedup actually fires.** Symfony's `SentMessage` adds the `Message-ID` header to a clone of the message, and `MailboxTransport` normalized the original, which never carries it. The column was always `null`, so `MessageStore::findIdByMessageId()` and the unique index were dead. The transport now passes the id from `SentMessage::getMessageId()` into `MessageNormalizer::normalize()` as an override, falling back to the header on the original when one was set explicitly. A message resent with the same Message-ID now updates the existing capture instead of creating a second one, and `CaptureService::store()` clears the earlier capture's attachments on that path so they no longer accumulate. Fixes #94.
...
- **PHPStan analysis on CI under Laravel 13.** Larastan boots the package through Testbench to resolve `view-string` arguments, and Testbench 11 only reads the root `composer.json` providers when running under its own CLI, so `MailboxServiceProvider` was never registered and `view-string` rejected `mailbox::app`. A `testbench.yaml.dist` now lists the provider explicitly, which Larastan passes through when it boots the application.
- **A capture failure no longer blocks real delivery in decorate mode.** `MailboxTransport::doSend()` ran capture unguarded before forwarding to the decorated transport, so a locked SQLite file, missing table, full disk, or unwritable attachment disk threw before `send()` and the real email was never delivered, contrary to what ARCHITECTURE.md promises. Capture is now wrapped in `try/catch`: with a decorated transport configured the failure is passed to `report()` and delivery continues; in capture-only mode it is still rethrown, since nothing else would deliver the message. Fixes #101.

### Changed
- **README and the Boost testing rule now document the test environment.** Laravel's default `phpunit.xml` sets `MAIL_MAILER=array`, so the transport never runs and every `Mailbox::assert*` fails, and the default `sqlite` store needs tables that a fresh checkout does not have. Both docs now carry a copy-pasteable `phpunit.xml` snippet using the `file` driver, note that the trait clears whatever store is configured and that attachments land on the `mailbox` disk unless faked, and explain that keeping the `sqlite` or `database` driver requires `mailbox:install` before the test run. Fixes #95.
